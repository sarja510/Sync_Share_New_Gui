...
require "MailCheck"
 ...
 #Pending user field will be included

	module UserCreation

		def UserCreation.AdminRegistration(driver,server)
		...
		def UserCreation.SelfRegistration(driver,server)


			#User sarja1 self registration
			driver.find_element(:link, "Create an account").click
			driver.find_element(:name, "name").send_keys("sarja1")
			driver.find_element(:xpath,'//input[@class="form-control input-sm ng-pristine ng-valid" and @ng-model="user.email"]').send_keys("sarja1@nilavodev.com")
			driver.find_element(:xpath,'//input[@class="form-control input-sm ng-pristine ng-valid" and @ng-model="user.confirmEmail"]').send_keys("sarja1@nilavodev.com")
			driver.find_element(:name,"password").send_keys("Sarja1")
			driver.find_element(:name,"confirmPassword").send_keys("Sarja1")
			sleep(10)
			driver.find_element(:xpath,'//button[@class="btn btn-primary btn-sm" and @ng-disabled="!form.$dirty" and @type="submit"]').click
			sleep(15)
			...
			#MailCheck.AccountConfirmation(@maildriver,"sarja8","1234")
